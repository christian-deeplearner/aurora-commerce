...
| S20 | Live Demo 1b — scaffold + run | `demo-1b-scaffold-run.gif` | `pnpm dev` → Aurora storefront on `:3000` + `/overview` on fake data |
...
- **Source:** record against the pre-built `feat/aurora-pdp-editorial` branch so
...
- **Crop:** terminal + relevant browser pane only; warm paper background where
  possible so the fallback reads as part of the Aurora deck, not a raw capture.
