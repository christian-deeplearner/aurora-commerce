# Aurora Workshop Deck — "How to Become a 100x Engineer"
...
real terminal beside it. The deck ships inside the same `aurora-commerce` repo
...
`index.html` is self-contained. Its `<style>` block mirrors the Aurora design
tokens from `src/app/globals.css` exactly — paper `#F4F1EA`, ink `#1A1A1A`,
hairline `#DCD8CF`, accent teal `#2F5D54` — with Instrument Serif for oversized
display headings and Geist Mono for labels and code. No hex colors are invented.
