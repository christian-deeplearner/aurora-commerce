# Aurora — Company Operating Doc

Aurora is a fictional editorial-luxury DTC house — "A House of Light" — that sells objects you keep. This repo is its operating system: a customer-facing **storefront** and an operator **command center**, one Next.js app over one deterministic fake-data layer.
...
A small, calm, AI-native commerce team. Humans set taste and make the calls; a named agent team does the legible, repeatable work and writes what it learns back into the memory base. We multiply judgment, we do not replace it.

## What

One Next.js 16 app, two surfaces, one seam:
- **Storefront** `app/(store)/` — editorial-luxury shopping: collections, PDPs, cart, checkout. Mirrors the restraint of a fashion-house manifesto site.
- **Command center** `app/(command)/` — operator console: overview KPIs, orders, products, customers, analytics.
...
We build for **Imogen** — see `knowledge-base/customer/icp.md`. She wants better and fewer, not more. She reads the care label before the price. She will not be rushed, flattered, or alarmed into a purchase. Every page and every line is written for her, not for "everyone."

## How-We-Talk

Spare, material, certain. The serif headline carries the weight; the body steps back. We state; we do not sell. Full voice — do/don't, banned phrases, worked examples — is the hot leaf below.

@knowledge-base/voice/brand-voice.md

## What-We-Refuse

- Real data of any kind. Aurora is fictional; the data is seeded faker output.
- Hype, urgency theater, and corporate slop in customer copy.
- **Somatic / wellness language** anywhere customer-facing. Hard rule. (See voice + PDP gate.)
...
| Styling | Tailwind CSS v4 — tokens in `src/app/globals.css` (paper / ink / accent). Never invent hex. |
...
- **Memory base taxonomy:** `knowledge-base/{voice,customer,standards,decisions,learnings,index.md}`. Append decisions and learnings as you work; the base compounds.
...
The cold map of the rest of the memory base (read on request, not auto-loaded): `knowledge-base/index.md`. Keep hot context small; move the rest cold.
...
- Don't write hype, urgency theater, or corporate slop — and **no somatic / wellness language** in customer copy. Ever.
- Don't put real data anywhere. Aurora is fictional; data is seeded.
- Don't bypass the fake-data seam with inline data in components.
- Don't invent hex colors — use the Tailwind tokens in `globals.css`.
