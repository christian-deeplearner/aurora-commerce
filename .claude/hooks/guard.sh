#!/usr/bin/env bash
# Aurora PreToolUse guard — guardrails in the brief, not in your nerves.
...
if printf '%s' "$cmd" | grep -Eq 'vercel.*(--prod|deploy)'; then
  echo "NOTE: this is a PRODUCTION deploy. Confirm the verification gate is green (pnpm build + pnpm test:e2e) before shipping. (Aurora /ship)" >&2
fi
