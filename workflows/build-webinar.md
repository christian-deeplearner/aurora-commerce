...
PHASE 2 — SURFACES  (parallel)
  • Aurora storefront  ∥  operator command center
...
`director` (orchestrate, never codes) · `tpm` (sequence + gates) · `cto` (architecture + build gate) · `fullstack-engineer` (surfaces + seam) · `ui-ux-designer` (Aurora design system) · `qa` (the Playwright proof). Named staff with job descriptions — not anonymous parallel processes. See `.claude/agents/`.
