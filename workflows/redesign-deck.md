...
- Extend the existing deck, do not rebuild: the deck CSS already mirrors the Aurora tokens in `src/app/globals.css`. Reuse `.label`, `.display`, `.row`, `.stat`, `.concept`, `.demo-badge`, `.fallback`, `.prompt-label`, `section.panel`, `section.motif`, `.cols`, `.card-note`, `.qr`.
