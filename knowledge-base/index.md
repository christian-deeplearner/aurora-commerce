...
> This is the **cold** map of Aurora's memory base. It is *not* auto-loaded. `CLAUDE.md` @-imports only the **hot** leaves (active voice, active standards); everything else lives here and is read on request.
>
...
- **Cold** (read on demand, listed below): customer notes, decision history, learnings, the rest of the map. An agent pulls these when the task calls for them.
...
| File | What it is | Tier |
|---|---|---|
| `customer/icp.md` | The seeded ideal customer (Imogen) | cold |
...
- **Naming:** `decisions/` and `learnings/` files are `YYYY-MM-DD-kebab-title.md`. Standards and voice are stable names (no date).
