# Decision — Aurora scaffold initialized

- **Date:** 2026-06-03
- **Status:** accepted
- **Owners:** director, cto
- **Context:** First entry in the Aurora decision log. The point of a decision log is that the next agent — or the next model — can read *why*, not just *what*.

## Decision

Stand up `aurora-commerce` as a single Next.js 16 App Router app: a customer-facing **Aurora storefront** and an operator **command center**, sharing one deterministic fake-data layer behind a clean seam. Ship it with a real memory base (`knowledge-base/`) and a real Claude Code harness (`.claude/`), authored to be the teachable artifact a webinar can reveal.
...
| Styling | Tailwind v4, tokens in `src/app/globals.css` (paper/ink/accent) |
| Brand | Aurora — fictional editorial-luxury DTC, "A House of Light" |
