...
| II | CONCEPTS | The mechanics, each as a worked mental model *before* touching Aurora: dynamic workflows, agent teams, remote control, the markdown/Obsidian memory base, then the synthesis. |
| III | PRACTICE | Build Aurora with the real harness — the named agent team running `workflows/new-feature.md` over the memory base, verified by Playwright. Every prompt **verbatim** from a named repo file. Motif **paid off**. |
| IV | CLOSE | The meta-reveal (`build-webinar.md`), the public quality bar = Aurora's own artifacts, motif **reprised**, CTA: clone + run `workflows/new-feature.md`. |

## Honesty rules locked into the rework

- Aurora `workflows/*.md` are **gated process specs**; `agent()`/`parallel()` is the Claude dynamic-workflow **JS feature**. Teach both, label each, never conflate.
...
- The public quality bar is **Aurora's own artifacts**. No private design reference is referenced or rendered in the deck.
