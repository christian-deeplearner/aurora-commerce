// ─────────────────────────────────────────────────────────────────────────
// Mock auth for the Aurora command center.
//
// This is a DEMO gate, not real account security: a single shared password
// ("aurora") opens a signed, httpOnly session cookie so the operator surfaces
...
export const DEMO_PASSWORD = "aurora";
...
const SECRET = new TextEncoder().encode(
  process.env.AURORA_SESSION_SECRET ?? "aurora-demo-session-secret-not-for-production",
);

const ISSUER = "aurora-command";
const AUDIENCE = "aurora-operator";
